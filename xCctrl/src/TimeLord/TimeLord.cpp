...
TimeLord::TimeLord(){
		...
		longitude=-82.0;
		timezone=-300;
		SUNRISE_DAWN_ANGLE = DAWN_NORMAL;
...
bool TimeLord::Position(double/*float*/ lat, double/*float*/ lon){
...
double TimeLord::Angle(double da) {
	if (0 != da)
		SUNRISE_DAWN_ANGLE = da;
	return SUNRISE_DAWN_ANGLE;
};
...
double/*float*/ TimeLord::MoonPhase(uint8_t * when){
// the period is 29.530588853 days
// we compute the number of days since Jan 6, 2000
// at which time the moon was 'new'
	long d;
	double/*float*/ p;
        double/*float*/ fracDay = (((when[tl_hour] * 60.0 + when[tl_minute]) * 60.0) + when[tl_second]) / 86400.0;
...
bool TimeLord::ComputeSun(uint8_t * when, bool rs) {
  uint8_t  month, day;
  double/*float*/ y, decl, eqt, ha, lon, lat;//, z;
  uint8_t a;
  int /*doy,*/ minutes;
  ...
  //compute hour angle
  ha=(cos(SUNRISE_DAWN_ANGLE) / (cos(lat)*cos(decl)) -tan(lat) * tan(decl)  );   
